...
def capture_webcam():
    ...
    if capturing and image_number <= 5:
        ...
        try:
            ...
            cursor.execute("UPDATE Offenders SET ImageData1 = ? WHERE OffenderId = ?", binary_data, offender_id)
            connection.commit()

            cursor.close()
            connection.close()

            messagebox.showinfo("Capture Image", f"Image {image_number} captured and saved to the database.")
            image_number += 1
